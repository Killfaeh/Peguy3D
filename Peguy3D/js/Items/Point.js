var Points = 
{
    ...
    {
        var dist2 = ($point2[0]-$point1[0])*($point2[0]-$point1[0]) + ($point2[1]-$point1[1])*($point2[1]-$point1[1]) + ($point2[2]-$point1[2])*($point2[2]-$point1[2]);
    ...
    createFibonacciPattern: function($width, $height, $nbDots, $random)
    {
        ...
        for (var i = 0; i < $nbDots; i++)
        {
            ...
            pointsList.push([point.x + Math.random()*2.0*random - random, point.y + Math.random()*2.0*random - random, 0.0]);
        }
